export const practicum = [
  {
    "id": 1,
    "owner": "practicum",
    "message": "Practicum by Yandex is a kind of boot camp. Our goal is to help ambitious and passionate people to master it and find their first job. At the same time, the level of our training allows graduates to carry out high-quality real projects of companies under the review of experienced mentors.",
    "direction": "row",
    "background-color": "orange",
    "text": "white",
    "bacground-image": "image",
  },
  {
    "id": 2,
    "owner": "user",
    "message": "Who will solve the problems? How much does it cost?",
    "direction": "row-reverse",
    "background-color": "white",
    "text": "black",
    "bacground-image": "null",
  },
  {
    "id": 3,
    "owner": "practicum",
    "message": "It is a win-win situation: we take real tasks from real companies and give them to graduates of Data Science and Data Analytics courses. The results of these tasks will be repeatedly checked by curators and returned to the customer as ordered for free.",
    "direction": "row",
    "background-color": "orange",
    "text": "white",
    "bacground-image": "image",
  },
  {
    "id": 4,
    "owner": "user",
    "message": "Why is it free?",
    "direction": "row-reverse",
    "background-color": "white",
    "text": "black",
    "bacground-image": "null",
  },
  {
    "id": 5,
    "owner": "practicum",
    "message": "Because Practicum has students who are eager to gain real life experience, find them interesting and useful, and are ready to take on your tasks",
    "direction": "row",
    "background-color": "orange",
    "text": "white",
    "bacground-image": "image",
  },
  {
    "id": 6,
    "owner": "user",
    "message": "Wow! I'm ready to start!",
    "direction": "row-reverse",
    "background-color": "white",
    "text": "black",
    "bacground-image": "null",
  },

]
